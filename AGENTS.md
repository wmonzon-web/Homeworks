...
- It is a child company of Xtreme Build Group. The owner supplied the Homeworks logo reference to recreate; never reuse the parent company's copy, photos, or contact details.
...
- Homeworks logo: owner-supplied reference recreated as HOME (off-white) / WORKS (deep red), two thin rules, and supporting lines; full and compact SVGs in `public/brand/`, generated by `npm run logo`, used in the header/footer and structured data.
...
- `scripts/generate-logo.mjs`: generates outlined SVG logos from the supplied reference layout (requires Arial Black and Arial Bold installed on Windows).
...
npm run logo                # regenerate full and compact logo SVGs and favicon
