...
const EMPTY: Draft = { services: [], urgency: "", propertyType: "", ownership: "", sqft: "", budget: "", name: "", email: "", phone: "", zip: "", notes: "", product: "" };
const STEPS = ["Service", "Timing", "Your home", "Photos", "Budget", "Contact"] as const;
...
export default function QuoteWizard() {
  ...
  useEffect(() => {
    ...
    if (service && (service === "other" || SERVICE_CATALOG.some((s) => s.slug === service)) && !next.services.includes(service)) {
      next.services = [...next.services, service];
    }
    for (const key of ["zip", "phone", "product"] as const) {
  ...
  const addPhotos = (list: FileList | null) => {
    if (!list) return;
    const incoming = [...list].filter((f) => f.type.startsWith("image/") && f.size <= MAX_BYTES);
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    for (const key of ["urgency", "propertyType", "ownership", "sqft", "budget", "name", "email", "phone", "zip", "notes", "product"] as const) {
      ...
      <motion.div key={step} initial={shouldAnimate ? { opacity: 0, x: direction * 10 } : false} animate={{ opacity: 1, x: 0 }} transition={{ duration: shouldAnimate ? 0.2 : 0, ease: MOTION_EASE }}>
        ...
        {step === 3 && (
          <fieldset>
            ...
            {photos.length > 0 && (
              <ul className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-5" aria-label="Selected photos">
                {photos.map((p, i) => (
                  <li key={`${p.name}-${i}`} className="relative">
                    <img src={URL.createObjectURL(p)} alt="" className="aspect-square w-full rounded-lg object-cover" />
                    <button type="button" onClick={() => setPhotos((list) => list.filter((_, j) => j !== i))} className="absolute -top-2 -right-2 inline-flex size-8 items-center justify-center rounded-full bg-ink text-white shadow-raised before:absolute before:-inset-2" aria-label={`Remove photo ${i + 1}`}>
            ...
            <div className="mt-8 flex gap-3">
              <Button type="button" size="lg" onClick={next}>{photos.length ? "Continue" : "Skip for now"}</Button>
            </div>
          </fieldset>
        )}
        ...
        {step === 5 && (
          <fieldset>
            ...
            <h2 id="wizard-heading" ref={headingRef} tabIndex={-1} className="text-3xl leading-[1.02] outline-none md:text-4xl">Where should we send the quote?</h2>
